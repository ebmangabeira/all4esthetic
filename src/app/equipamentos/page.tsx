							...
							<div className="row align-items-center">
								<div>
									...
									<p className="about-description">
										A nossa missão é fornecer soluções seguras, eficazes e adaptadas às necessidades de cada cliente ou clínica, promovendo bem-estar e confiança.
									</p>
								</div>
							</div>
						</div>
							...
							<div className="row">
										...
										<ul className="list-unstyled">
											<li className="form-check">
												<input
													...
													onChange={() => { setAvailability("Venda"); setRental(null); }}
												/>
												<label className="form-check-label" htmlFor="filter-venda">Venda</label>
											</li>
											<li className="form-check">
												<input
													type="radio"
												...
												/>
												<label className="form-check-label" htmlFor="filter-aluguer">Aluguer</label>
											</li>
										</ul>
									</div>
									...
									{mobileOpen && (
										...
										>
											<div className="mobile-filters-content">
												<h2 id="mobile-filters-title" className="visually-hidden">Filtros</h2>
													...
													<ul className="list-unstyled">
														<li className="form-check">
															<input
																...
																onChange={() => { setAvailability("Venda"); setRental(null); }}
															/>
															<label className="form-check-label" htmlFor="filter-venda-mob">Venda</label>
														</li>
														<li className="form-check">
															<input
																type="radio"
																...
																onChange={() => setAvailability("Aluguer")}
															/>
															<label className="form-check-label" htmlFor="filter-aluguer-mob">Aluguer</label>
														</li>
													</ul>
												</div>
								...
								<div className="col-lg-9">
									<div
										...
										className={`selected-tags-bar d-md-none ${(!selectedCats.length && !availability) ? "d-none" : ""}`}
									>
										{selectedCats.map((c) => (
											<span key={`tag-${c}`} className="selected-tag">
												{c}
												<button className="close-btn" onClick={() => clearTag(c)} aria-label={`Remover ${c}`}>X</button>
											</span>
										))}
										...
										{availability && (
											<span className="selected-tag">
												{availability}
												<button className="close-btn" onClick={() => clearTag(availability)} aria-label="Remover disponibilidade">X</button>
											</span>
										)}
										...
										{availability === "Aluguer" && rental && (
											<span className="selected-tag">
												{RENT_LABEL[rental]}
												<button className="close-btn" onClick={() => clearTag(rental)} aria-label="Remover período">X</button>
											</span>
										)}
									</div>
									...
									<div id="equipamentos-container" className="row g-4">
												...
												>
													<div className="card equipamento-card h-100">
														...
														<div className="card-body d-flex flex-column">
															...
															<p className="card-id text-muted small mb-2">{e.id}</p>
										...
										{!slice.length && (
											<div className="col-12">
												<p className="text-center text-muted">Nenhum equipamento encontrado com os filtros selecionados.</p>
											</div>
										)}
									</div>

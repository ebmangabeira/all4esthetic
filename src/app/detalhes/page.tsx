...
function normalizeFunctions(raw?: FuncItem[]): Array<{ title: string; sub?: string }> {
	...
	return raw
		.map((item) => {
			...
			if (item && typeof item === "object") {
				const o = item as any;
				const title =
					o.nome ?? o.name ?? o.descricao ?? o["descrição"] ?? o.description ?? "";
				const sub = o.descricao ?? o["descrição"] ?? o.description ?? undefined;
...
function findTechnicalBlock(tree?: Record<string, unknown>): KV | undefined {
	...
	for (const [k, v] of Object.entries(tree)) {
		const nk = norm(k);
		if (want.some((w) => nk.includes(w)) || nk.includes("tecnic")) {
			return v as KV;
		}
	}
	...
	for (const [k, v] of Object.entries(tree)) {
		const nk = norm(k);
		if (nk.includes("dadostecnicos") || nk.includes("fichatecnica")) {
			return v as KV;
		}
	}
...
function toRows(data?: KV): Array<[string, string]> {
	...
	const push = (k: string, v: unknown) => {
		if (isPrimitive(v)) out.push([k, asString(v)]);
		else if (Array.isArray(v)) {
			if (v.every(isPrimitive)) out.push([k, (v as any[]).map(asString).join(" · ")]);
			else {
				v.forEach((item, idx) => {
					if (isTuple(item)) out.push([k || `Item ${idx + 1}`, `${item[0]}: ${item[1]}`]);
						...
						const vv =
							...
							o["descrição"] ??
							o.description;
						if (isPrimitive(vv) && typeof kk === "string") out.push([kk, asString(vv)]);
						else flatten(o, k);
					}
				});
			}
	...
	const flatten = (obj: Record<string, unknown>, parent?: string) => {
		...
		if (!added && parent) out.push([parent, "—"]);
	};
	...
	if (Array.isArray(data)) {
		data.forEach((item, i) => {
			if (isTuple(item)) out.push([item[0], item[1]]);
			else if (isPlainObject(item)) flatten(item);
			else if (isPrimitive(item)) out.push([`Item ${i + 1}`, asString(item)]);
		});
...
export default function Page() {
	...
	const designation = eq?.["designação"] ?? eq?.designacao ?? "";
	...
	const functions = normalizeFunctions(eq?.funcoes ?? eq?.functions);

	const techBlock = findTechnicalBlock(eq as any);
	const techRows = toRows(techBlock);
							...
							<div className="row">
								<div className="grid-product">
									...
									<div className="info-column">
										<h2 id="pd-title-text" className="pd-product-title">{title}</h2>
										{designation ? (
											<p className="pd-designacao" style={{ marginTop: "-18px" }}>
												{designation}
											</p>
										) : null}
										<div
												...
												<div id="pd-functions-list">
													{functions.length ? (
														<ul className="list-unstyled" style={{ marginBottom: 0 }}>
															{functions.map((f, i) => (
																<li key={i} style={{ marginBottom: "10px", color: "var(--heading-color)" }}>
																	<div>{f.title}</div>
																	{f.sub ? (
																		<div style={{ fontSize: "0.95rem", opacity: 0.9 }}>{f.sub}</div>
																	) : null}
																</li>
															))}
														</ul>

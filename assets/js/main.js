;(function(){
  ...
  function toggleScrolled(){
    ...
    if (!header || (!header.classList.contains("scroll-up-sticky") && header.classList.contains("sticky-top") === false && header.classList.contains("fixed-top") === false)) return;
  ...
  function initSwiper(){
    document.querySelectorAll(".init-swiper").forEach(swiperEl=>{
      ...
      if (!cfgEl) return;
      let cfg;
      try { cfg = JSON.parse(cfgEl.textContent.trim()); } catch { return; }
      swiperEl.classList.contains("swiper-tab") ? initSwiperWithCustomPagination(swiperEl, cfg) : new Swiper(swiperEl, cfg);
    });
  }
...
document.addEventListener("DOMContentLoaded", async ()=>{
  let allEquip = [];
  try { allEquip = await (await fetch("assets/data/equipamentos.json")).json(); } catch {}
  ...
  (function enforcePanelScrollBehavior(){
    if (mobFilt) {
      mobFilt.style.overflow = "hidden";
      mobFilt.style.webkitOverflowScrolling = "auto";
    }
    const panel = mobFilt?.querySelector(".mobile-filters-content");
    if (panel){
  ...
  function scrollResultsIntoView(){
    if (!mobFilt || mobFilt.classList.contains("open")) return;
  ...
  function renderPagination(){
    ...
    pg.append(
      mk(page===1,"«",()=>{
        page--; renderPage(); renderPagination(); scrollResultsIntoView();
      }),
      ...Array.from({length:cnt},(_,i)=>
        mk(false,String(i+1),()=>{
          page=i+1; renderPage(); renderPagination(); scrollResultsIntoView();
        }, i+1===page?"active":"")
      ),
      mk(page===cnt,"»",()=>{
        page++; renderPage(); renderPagination(); scrollResultsIntoView();
  ...
  function apply({cats,av,rents}){
    ...
    filtered=allEquip.filter(e=>{
      ...
      if (av==="Aluguer") return rents.length? rents.some(r=>e.rental[r]) : Object.values(e.rental).some(Boolean);
      return true;
    });
  ...
  function openPanel() {
    ...
    mobFilt.setAttribute("aria-hidden","false");
    lockBody();
    const panel = mobFilt.querySelector(".mobile-filters-content");
  ...
  function closePanel() {
    ...
    mobFilt.setAttribute("aria-hidden","true");
    unlockBody();
  }

  btnFiltro && btnFiltro.addEventListener("click", openPanel);
  ...
  function trapScroll(el){
    ...
    el.addEventListener("touchmove", e=>{
      if (!mobFilt.classList.contains("open")) return;
  ...
  const panelScrollable = mobFilt?.querySelector(".mobile-filters-content");
  trapScroll(panelScrollable);
...
document.addEventListener("DOMContentLoaded", async ()=>{
  ...
  if (!id) return;
  let data = [];
  try { data = await (await fetch("assets/data/equipamentos.json")).json(); } catch {}
  ...
  availUL.innerHTML="";
  eq.sale && availUL.insertAdjacentHTML("beforeend", "<li><i class='bi bi-check-circle-fill me-2'></i>Para venda</li>");
  ...
  Object.entries(eq.rental).filter(([,ok])=>ok).forEach(([p])=>{
    availUL.insertAdjacentHTML("beforeend", `<li><i class='bi bi-check-circle-fill me-2'></i>Para aluguer ${labels[p]||p}</li>`);
  });
  ...
  if (track && eq.images){
    eq.images.forEach((src,idx)=>{
      ...
      const img=Object.assign(document.createElement("img"),{ src, alt:`Miniatura ${idx+1}`, loading:"lazy" });
      li.append(img);
      li.addEventListener("click",()=>{
        const mainImg2 = document.getElementById("pd-main-img");
        if(!mainImg2) return;
        mainImg2.style.opacity="0";
        mainImg2.addEventListener("transitionend",function h(e){
          if (e.propertyName==="opacity"){
            mainImg2.removeEventListener("transitionend",h);
            mainImg2.src=src;
